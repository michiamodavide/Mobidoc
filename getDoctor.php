...
while($rows = mysqli_fetch_array($result)){
  ...
  if (isset($_SESSION['doctor_email'])) {
    $sql2 = "select * from doctor_profile where email ='".$doctor_email."' AND active = 'Y'";
  }else{
    $sql2 = "select * from doctor_profile where email ='".$doctor_email."' AND puo_refertare='N' AND active = 'Y'";
  }
